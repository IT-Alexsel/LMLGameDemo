...
public:
	SkeletonEnemy()
	{
		...
		static SDL_Texture* texture = texture_map.find(ResID::Tex_Skeleton)->second; // 骷髅纹理
		static SDL_Texture* texture_sketch = texture_map.find(ResID::Tex_SkeletonSketch)->second; // 骷髅受击纹理
		// 数据
		static ConfigManager::EnemyTemplate& tem = ConfigManager::GetInstance()->skeleton_template;
		max_hp = tem.hp;
		hp = max_hp;
		max_speed = tem.speed;
		speed = tem.speed;
