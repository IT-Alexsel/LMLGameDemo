...
public:
	...
	{ 
		...
		static SDL_Texture* texture = texture_map.find(ResID::Tex_Slim)->second; // 史莱姆纹理
		static SDL_Texture* texture_sketch = texture_map.find(ResID::Tex_SlimSketch)->second; // 史莱姆受击纹理
		// 数据
		static ConfigManager::EnemyTemplate& tem = ConfigManager::GetInstance()->slim_template;
		max_hp = tem.hp;
        hp = max_hp;
        max_speed = tem.speed;
		speed = tem.speed;
