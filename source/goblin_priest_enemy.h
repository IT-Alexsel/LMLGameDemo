...
public:
	GoblinPriestEnemy()
	{
		...
		static SDL_Texture* texture = texture_map.find(ResID::Tex_GoblinPriest)->second; // 哥布林祭祀纹理
		static SDL_Texture* texture_sketch = texture_map.find(ResID::Tex_GoblinPriestSketch)->second; // 哥布林祭祀受击纹理
		// 数据
		static ConfigManager::EnemyTemplate& tem = ConfigManager::GetInstance()->goblinPriest_template;
		max_hp = tem.hp;
		hp = max_hp;
		max_speed = tem.speed;
		speed = tem.speed;
